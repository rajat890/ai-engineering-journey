...
# image_uris.retrieve looks up AWS's pre-built SKLearn serving container
# for this framework version -- same container real deployments would use.
# We only need its URI (a string), not to run it, so this works in local
# mode with no quota required.
...
register_processor = SKLearnProcessor(
    framework_version="1.2-1",
    role=role,
    instance_type="local",       # <-- registration script itself runs locally
    instance_count=1,
...
step_register = ProcessingStep(
    ...
    # job_arguments passes real values (including the not-yet-known S3 path,
    # resolved automatically at execution time) as CLI args to the script --
    # same pattern as passing --model-s3-uri to any argparse script.
    job_arguments=[
        "--model-s3-uri", step_train.properties.ModelArtifacts.S3ModelArtifacts,
        "--image-uri", sklearn_inference_image,
        "--model-package-group-name", "aria-classifier-models",
    ],
)
